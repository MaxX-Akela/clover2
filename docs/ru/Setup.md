...
5. Подключите полетный контроллер к компьютеру по USB.
6. Выберите последовательный порт полётного контроллера.
...
Переподключение полётного контроллера для загрузки прошивки
```
<br>

7. Дождитесь, пока QGroundControl обнаружит полётный контроллер, и выберите *PX4 Flight Stack*.
8. Откройте *Advanced settings*.
9. В выпадающем меню выберите *Custom firmware file...*. Нажмите *OK*.
...
10. Выберите ранее скачанный файл прошивки.
...
6. Подтвердите применение настроек *Apply*.
...
После изменения параметров нажмите Save. При необходимости перезагрузите полетный контроллер.

```{warning}
Не изменяйте значения ПИД-регуляторов без необходимости. Используйте рекомендованные значения для установленной конфигурации.
```
