...
8. Дождитесь, пока QGroundControl обнаружит полётный контроллер, и выберите *PX4 Flight Stack*.
...
```{tip}
Необходимо учитывать, что для идеального полета параметры PID-регуляторов подбираются вручную для каждого конкретного собранного квадрокоптера.
```
