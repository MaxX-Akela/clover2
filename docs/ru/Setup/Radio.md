...
```
После устранения предупреждения перейдите в главное меню аппаратуры.
...
Рекомендуемый способ сопряжения — задать одинаковую **Binding Phrase** на передатчике и приёмнике. Настройка через Web UI поддерживается в ExpressLRS 3.0 и новее. Если на одном из устройств установлена более ранняя версия, сначала обновите его через [ExpressLRS Web Flasher](https://expresslrs.github.io/web-flasher/).

### Настройка передатчика

1. Включите аппаратуру. Нажмите кнопку **SYS**.
...
```{caution}
Если вместе с полётным контроллером получает питание видеопередатчик, подключите к нему антенну и обеспечьте охлаждение либо временно отключите VTX. Не устанавливайте пропеллеры.
```
...
### Альтернативный способ: режим Bind
